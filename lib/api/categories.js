...
exports.find = function(req, res, next){
  ...
  req.app.db.models.Category.pagedFind({
    ...
    if (req.xhr) {
      ...
      results.filters = req.query;
      let template = req.app.utils.loadTemplate('admin/categories/index.jade')
      res.write(template({ data: { results: escape(JSON.stringify(results)) } }))
      res.end()
    }
  });
};
  ...
  req.app.db.models.Category.findById(req.params.id).exec(function(err, category) {
    ...
    if (req.xhr) {
      res.send(category);
        ...
        let template = req.app.utils.loadTemplate('admin/categories/details.jade')
        res.write(template({ data: { record: escape(JSON.stringify(category)) } }))
        res.end()
    }
  });
};
...
exports.create = function(req, res, next){
  ...
  workflow.on('duplicateCategoryCheck', function() {
    req.app.db.models.Category.findById(slugify(req.body.pivot +' '+ req.body.name)).exec(function(err, category) {
      if (err) {
  ...
  workflow.on('createCategory', function() {
    var fieldsToSet = {
      _id: slugify(req.body.pivot +' '+ req.body.name),

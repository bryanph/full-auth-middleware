...
exports.read = function(req, res, next){
    ...
    var asyncFinally = function(err, results) {
        ...
        if (req.xhr) {
            ...
            let template = req.app.utils.loadTemplate('admin/administrators/details.jade')
            res.write(template({
                data: {
...
exports.linkUser = function(req, res, next){
    ...
    workflow.on('verifyUser', function(callback) {
        req.app.db.models.User.findOne({ username: req.body.username }, 'username').exec(function(err, user) {
            if (err) {
                return workflow.emit('exception', err);
            }
